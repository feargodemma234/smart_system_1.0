...
# ========== CSS - MINIMIZED FLOATING AI ==========
st.markdown("""
<style>
.stApp {background: linear-gradient(135deg, #050510 0%, #120A2E 100%); color: #E0E0FF; padding-bottom: 80px;} /* padding so button doesn't cover content */
...
.post-card,.chat-bubble {background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(15px); border: 1px solid rgba(168, 85, 247, 0.3); border-radius: 15px; padding: 12px; margin-bottom: 8px; font-size: 14px;}
.ai-bubble {border: 1px solid #00F0FF; background: rgba(0, 240, 255, 0.1);}
.me {background: linear-gradient(90deg, #A855F7 0%, #00F0FF 100%); border: none; margin-left: 15%;}
...
def call_groq_api(api_key, messages):
    ...
    try:
        ...
        return f"⚠️ Error: Check Groq API key"
...
# ========== MAIN APP ==========
    ...
    if st.session_state['ai_open']:
        ...
        ai_input = st.text_input("", key="ai_input", placeholder="Ask anything...")
        colA, colB = st.columns([4,1])
        with colA:
            if st.button("Send", key="ai_send", use_container_width=True):
                if api_key and ai_input:
                    ...
                    st.rerun()
        with colB:
            if st.button("X", key="ai_close"): st.session_state['ai_open'] = True; st.session_state['ai_open'] = False; st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)