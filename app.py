...
div.stButton > button[key="ai_toggle_btn"] {
    position: fixed!important; bottom: 15px!important; right: 15px!important; z-index: 9999!important;
    background: linear-gradient(90deg, #A855F7 0%, #00F0FF 100%)!important; color: white!important;
    border: none!important; border-radius: 50%!important; width: 50px!important; height: 50px!important;
    font-size: 22px!important; box-shadow: 0 0 15px rgba(0, 240, 255, 0.6)!important; padding: 0!important;
}
.ai-float-window {
    position: fixed; bottom: 75px; right: 15px; width: 300px; max-width: 85vw; height: 400px; max-height: 65vh;
    z-index: 9998; background: rgba(10, 5, 30, 0.98); backdrop-filter: blur(25px);
    border: 1px solid rgba(0, 240, 255, 0.4); border-radius: 18px; padding: 10px; display: flex; flex-direction: column;
}
.ai-chat-history {flex: 1; overflow-y: auto; margin-bottom: 8px;}
...
def call_groq_api(api_key, messages):
    ...
    except: return f"⚠️ Error: Check Groq API key at console.groq.com"
    ...
    with col2:
        if st.session_state['chat_with']:
            ...
            for post in get_posts():
                ...
                with colp2:
                    if username == st.session_state['username']: # DELETE BUTTON
        ...
        with colA:
            if st.button("Send", key="ai_send", use_container_width=True):
                if not api_key: st.warning("Add Groq API key in sidebar")
                    ...
                    st.rerun()
        with colB:
            if st.button("🗑️", key="ai_clear"): clear_ai_chat(st.session_state['username']); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)