...
# ========== POST + DM + LIKE + COMMENT FUNCTIONS ==========
...
# ========== MAIN APP WITH SIDEBAR ==========
    ...
    with st.sidebar:
        ...
        with tab2:
            ...
            search_query = st.text_input("🔍 Search users...", placeholder="Type name or email")
            users = get_all_users(search_query)
            if not users: st.caption("No users found")
            for u in users:
    ...
    with col2:
        if st.session_state['chat_with']:
            # PRIVATE CHAT
            st.markdown('<div class="glass-card">', unsafe_allow_html=True)
            st.write(f"### Private Chat with {st.session_state['chat_with']}")
            ...
            with chat_container:
                ...
                if not messages: st.caption("No messages yet. Say hi!")
                for msg in messages:
                    sender = msg[1]
            ...
            for post in posts:
                ...
                with colC:
                    st.markdown('<div class="small-btn">', unsafe_allow_html=True)
                    st.button(f"💬 Comment", key=f"commentbtn{post_id}", disabled=True) # just for UI
                    st.markdown('</div>', unsafe_allow_html=True)
